...
class StarCraft2Env(MultiAgentEnv):
    ...
    def unit_max_shield(self, unit):
        ...
        if ((unit.unit_type == Protoss.Stalker and unit.owner == 2) or
            (unit.unit_type == self.stalker_id) and unit.owner == 1):
            return 80  # Protoss's Stalker
        if ((unit.unit_type == Protoss.Zealot and unit.owner == 2) or
            (unit.unit_type == self.zealot_id) and unit.owner == 1):
            return 50  # Protoss's Zaelot
        if ((unit.unit_type == Protoss.Colossus and unit.owner == 2) or
            (unit.unit_type == self.colossus_id) and unit.owner == 1):
    ...
    def get_unit_type_id(self, unit, ally):
        ...
        if ally:  # use new SC2 unit types
            ...
            # Uncomment if you're trying to evaluate 3s5z trained on custom map in the standard map
            # if unit.unit_type == 1935:
            #     type_id = 1
            # else:
            #     type_id = 0
    ...
    def _init_ally_unit_types(self, min_unit_type):
        ...
        self.id2name[self.marine_id] = 'Marine'
        self.id2name[self.marauder_id] = 'Marauder'
        self.id2name[self.medivac_id] = 'Medivac'
        self.id2name[self.stalker_id] = 'Stalker'
        self.id2name[self.zealot_id] = 'Zealot'
        self.id2name[self.colossus_id] = 'Colossus'
        self.id2name[self.zergling_id] = 'Zergling'
        self.id2name[self.baneling_id] = 'Baneling'
        self.id2name[self.hydralisk_id] = 'Hydralisk'
